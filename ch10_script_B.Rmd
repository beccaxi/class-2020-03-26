...
avg_train <- train %>% 
  group_by(treatment) %>% 
  select(att_start, att_end, att_chg) %>% 
  summarize(
...
# Second (in a new code chunk), and just as we did with income, calculate the difference in means between  treated and the controls for only `att_chg`. (You will want to get those values on the same row, so that subtraction can be done.) Does the difference seem "large?" Do the numbers seems sensible? Provide an hypothesis as to why the average `att_chg` is not zero in both groups.

diff_avg_train <- avg_train %>% 
  select(treatment, avg_att_chg) %>% 
  pivot_wider(names_from = treatment, values_from = avg_att_chg) %>% 
  summarize(diff_avg_att_chg = Control - Treated)

# Unusual how avv_att_chg in control group is not zero. Perhaps other factors we are not aware of bc not investigated in the study.
...
train_conf_99 <- train %>% 
  ...
  pivot_wider(names_from = treatment, values_from = avg_att_chg) %>% 
  summarize(diff_avg_att_chg = Control - Treated) %>% 
  pull(diff_avg_att_chg) %>% 
  quantile(c(0.005, 0.995))

```

Bayesian: 99% chance that the difference between average incomes in Treated and Control groups for the entire population would fall within this interval.F
Frequentist: 99% of the time, this method of bootstrap sampling rebeyields the true mean.
...
The units are the participants. The treatment is exposing participants to Spanish speakers on the train platform. The outcomes are changes in attitudes after the treatment is applied. The causal effect we want to measure is the possible change in attitude induced by the treatment. The fundamental problem of causal inference is not knowing whether there is definitely a causal effect linking treatment to attitude changes.
...
train_plot <- train %>% 
  ...
  ggplot(aes(x = treatment, y = att_chg)) +
    ...
    geom_hline(yintercept = -0.40625) +
    geom_hline(yintercept = 0.39216)
